...
(function () {
    ...
    /* ============================================================
       ...
       ============================================================ */
    var STATS_API_DEFAULT = 'https://i-love-you-anya.onrender.com';   // адрес сервиса статистики
    ...
    function stats() {
        ...
        var serviceNote = null;      // причина, о которой сообщил сервис
        ...
        function render(d, stale) {
            ...
            // сервис ещё не подключён к Telegram — не рисуем нули как настоящие цифры,
            // а показываем причину, которую сообщил сам сервис
            if ((d.status === 'error' || d.status === 'login') && !total) {
                box.classList.add('is-error');
                serviceNote = d.error ||
                    'Сервис запущен и отвечает, но у него нет доступа к Telegram: ' +
                    'в Render не заданы API_ID, API_HASH и TG_SESSION.';
                if (ui.verdict) {
                    ui.verdict.textContent = d.login_required
                        ? 'статистика ждёт подключения Telegram'
                        : 'статистика ещё не подключена';
                }
                if (ui.hint) {
                    ui.hint.innerHTML = esc(serviceNote) +
                        (d.login_url
                            ? ' <a href="' + encodeURI(d.login_url) + '" target="_blank" rel="noopener">Открыть страницу входа</a>'
                            : '');
                }
                if (ui.when) ui.when.textContent = d.login_required ? 'нужен вход в Telegram' : 'нужны данные Telegram';
                return false;                 // пробуем запасной источник
            }
            ...
            if (ui.when) {
                ...
                if (d.status === 'static') ui.when.textContent = 'из выгрузки Telegram' + (when ? ' · ' + when : '');
                else if (d.status === 'syncing') ui.when.textContent = 'считаю историю…' + (d.progress && d.progress.processed ? ' (' + spaced(d.progress.processed) + ')' : '');
                else ui.when.textContent = stale ? 'данные от ' + when : 'обновлено ' + when;
            }
        ...
        // запасной источник: файл stats.json рядом с сайтом (получается из выгрузки Telegram)
        function fallback(reason) {
            return fetch('stats.json', { cache: 'no-store' })
                .then(function (r) { if (!r.ok) throw new Error('нет stats.json'); return r.json(); })
                .then(function (d) {
                    d.status = 'static';
                    render(d, true);
                    if (ui.hint) ui.hint.textContent =
                        'Это данные из файла stats.json (выгрузка Telegram). Живые цифры появятся, когда сервис получит доступ к Telegram.';
                })
                .catch(function () {
                    restore();
                    var cached = false;
                    try { cached = !!localStorage.getItem('stats_cache'); } catch (e) {}
                    if (cached || serviceNote) return;      // цифры или понятная причина уже на экране
                    fail(reason || 'Цифры не пришли.');
                });
        }

        function fail(msg) {
            box.classList.add('is-error');
            if (ui.when && ui.when.textContent !== 'нужны данные Telegram') ui.when.textContent = 'нет связи';
        ...
        function load() {
            ...
            fetch(base + '/api/stats', { signal: ctrl.signal, cache: 'no-store' })
                ...
                .then(function (d) {
                    if (render(d, false) === false) {
                        fallback('Сервису нужны ключи Telegram.');
                    }
                })
                .catch(function (e) {
                    var why = (e && e.name === 'AbortError')
                        ? 'Сервис не ответил за 90 секунд.'
                        : 'Цифры не пришли: ' + ((e && e.message) ? e.message : 'ошибка сети') + '.';
                    fallback(why);
                })
                .finally(function () {
